...
const Register = () => {
  ...
  const [errMsg, setErrMsg] = useState("");  // For showing error messages
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.status === 201) {
        setErrMsg({ status: "success", message: result.message });
        navigate("/login");  // Redirect to login page after successful registration
      } else {
        setErrMsg({ status: "failed", message: result.message });
      }
    } catch (error) {
      setErrMsg({ status: "failed", message: "Server error occurred" });
    } finally {
        ...
        <div className="w-full lg:w-1/2 h-full p-10 2xl:px-20 flex flex-col justify-center ">
          ...
          <form className="py-8 flex flex-col gap-5" onSubmit={handleSubmit(onSubmit)}>
            ...
            {isSubmitting ? (
              ...
              <CustomButton
                type="submit"
                containerStyles="inline-flex justify-center rounded-md bg-blue-700 px-8 py-3 text-sm font-medium text-black outline-none"
